...
from .parser import parse_block, seconds_to_tag
...
def run(block_text, cfg, dry_run=False, ask=input):
    ...
    for i, src in enumerate(sources, 1):
        ...
        try:
            ...
            print(f"FAILED: {msg}")
            if "instagram" in src.url.lower() and not cfg.get("cookies_browser"):
                print("Tip: Instagram usually needs login cookies. Run `bolt config` and set your browser.")
            failed.append({"url": src.url, "error": msg})
            continue

        for d in downloads:
            final, action, vcodec, res = make_premiere_ready(d["path"], ingest)
...
def main():
    ap = argparse.ArgumentParser(prog="bolt", description="Bolt Motivation footage ingest tool")
    ap.add_argument("input", nargs="?", help="'config', or a .txt file with the research block (default: paste mode)")
